...
public class Pedido {
	...
	private ItemDePedido[] itensDoPedido;
	...
	public Pedido(LocalDate dataPedido, int formaDePagamento) {
		...
		itensDoPedido = new ItemDePedido[MAX_PRODUTOS];
		quantProdutos = 0;
	...
	public boolean incluirProduto(ItemDePedido novo) {
		...
		if (quantProdutos < MAX_PRODUTOS) {
			itensDoPedido[quantProdutos++] = novo;
			return true;
		}
		return false;
	}
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += itensDoPedido[i].getProduto().valorDeVenda();
		}
	 ...
	 public void mesclarPedido(Pedido outroPedido){
		...
		* se for valido iterar sobre o vetor do outro pedido, se nao lança excecao
		*verificar se o produto é igual, caso seja apenas adiciona na quantidade
		 */
		ItemDePedido[] listaSecundaria = outroPedido.getItensDoPedido();

		if ((this.itensDoPedido.length + listaSecundaria.length) > MAX_PRODUTOS){
			throw new IllegalArgumentException("Capacidade maxima de produtos excedida");
		}

		for (int i = 0; i < quantProdutos; i++){
			if(listaSecundaria[i].equals(this.itensDoPedido[i])){
				this.itensDoPedido[i].setQuantidade(this.itensDoPedido[i].getQuantidade() + 1);
				if (itensDoPedido[i].getPrecoVenda() > listaSecundaria[i].getPrecoVenda()){
					double menorPreco = listaSecundaria[i].getPrecoVenda();
					itensDoPedido[i].setPrecoVenda(menorPreco);
				}
			} else{
				incluirProduto(listaSecundaria[i]);
			}
		}

		for (int i = 0; i< outroPedido.getQuantProdutos();i++){
			outroPedido.getItensDoPedido()[i] = null;
		}
	 ...
	 public void gerarReciboDeVenda(){
		...
		for (int i = 0; i< quantProdutos;i++){
			...
			double valorPrecoUnitario = itensDoPedido[i].getPrecoVenda()/itensDoPedido[i].getQuantidade();
			double valorSubtotal = itensDoPedido[i].getPrecoVenda() * itensDoPedido[i].getQuantidade();
			String nomeProduto = String.format("%s", itensDoPedido[i].getProduto().getDescricao());
       		String quantidadeProduto = String.format("%d",itensDoPedido[i].getQuantidade() ).trim();
			String precoUnitario =  String.format("%.2f",valorPrecoUnitario ).trim();
			String subTotal = String.format("%.2f",valorSubtotal).trim();

			System.out.printf("===== Produto %d ====", i+1 );
			System.out.printf("Nome do Produto: %s",nomeProduto);
			System.out.printf("Quantidade do Prodtuo: %s",quantidadeProduto);
			System.out.printf("Preço Unitario: %s",precoUnitario);
		...
		System.out.println("================================");
		System.err.printf("%s", this.toString());
	...
	public String toString() {
		...
		for (int i = 0; i < quantProdutos; i++ ) {
			stringPedido.append(itensDoPedido[i].getProduto().toString() + "\n");
		}
